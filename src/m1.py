...
class CircleChanger(object):
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        ################################################################
        if self.circle.radius + amount_to_swell_or_shrink < 0:
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        new_x = abs(self.circle.center.x + other_circle_changer.circle.center.x)/2
        new_y = abs(self.circle.center.y + other_circle_changer.circle.center.y)/2
        radius = self.get_distance_from(other_circle_changer.circle.center)/2
        circle_changer = CircleChanger(new_x, new_y, radius, 'red', self.colors + other_circle_changer.colors)
